...
public class HomeNet {
	...
	public static void Good_Moreshop(final FragmentActivity activity,
			final ImageView[] good_moreshop, final FinalBitmap fm) {
		FinalHttp fh = new FinalHttp();
		AjaxParams params = new AjaxParams();
		params.put("method", "find_goodmoreshop");
			fh.post(VARIABLE.BUYMORE_SHOP, params, new AjaxCallBack<Object>() {
				...
				public void onSuccess(Object t) {
					...
					super.onSuccess(t);
						...
						myApplication my = (myApplication) activity.getApplication();
						List<ShopInfo> list = my.list_goodshop;
						list = g.fromJson((String) t, typeOfT);
						my.setList_goodshop(list);
						for (ShopInfo s : list) {
							System.out.println("HomeNet:" + s.getF_c_Simagpath() + ","
									+ s.getF_c_Sname());
						}
						for (int i = 0; i < 3; i++) {
							fm.display(good_moreshop[i], VARIABLE.IMAGE_URL+list.get(i).getF_c_Simagpath());
						}
